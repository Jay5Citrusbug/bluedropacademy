...
export class ChatbotLoginPage {
    ...
    async login(email: string, password: string) {
        ...
        try {
            ...
            await this.page
                .getByRole("button", { name: chatbotLocators.loginWithEmailBtn.name })
                .click();

            console.log(`Filling in email: ${email}`);
            await this.page
                .getByRole("textbox", { name: chatbotLocators.emailInput.name })
                .fill(email);

            console.log("Filling in password...");
            await this.page
                .getByRole("textbox", { name: chatbotLocators.passwordInput.name })
                .fill(password);

            console.log('Clicking "Submit" to log in...');
            await this.page
                .getByRole("button", { name: chatbotLocators.submitLoginBtn.name })
                .click();
            ...
            const verifyTitle = iframeLocator.getByRole("heading", {
