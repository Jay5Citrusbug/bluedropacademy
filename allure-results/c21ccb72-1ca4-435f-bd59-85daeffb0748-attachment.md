...
- Name: BlueDrop test cases >> Hamburger Menu & History >> TC_12: Open Hamburger Menu
- Location: D:\Playwright\Bluedrop_academy\tests\Main.spec.ts:101:9
...
Call log:
  - waiting for locator('iframe[name="htmlComp-iframe"]').contentFrame().getByRole('button', { name: 'icon' }).nth(2)
    - locator resolved to <button class="bg-transparent border-0 p-0 m-0">…</button>
  - attempting click action
    - scrolling into view if needed
    - done scrolling

    at chatbotPage.OpenHamburgerMenu (D:\Playwright\Bluedrop_academy\tests\Pages\chatbotPage.ts:147:19)
    at D:\Playwright\Bluedrop_academy\tests\Main.spec.ts:102:7
```
