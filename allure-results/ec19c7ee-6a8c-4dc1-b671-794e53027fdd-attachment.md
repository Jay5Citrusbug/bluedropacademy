...
- Location: D:\Playwright\Bluedrop_academy\tests\Main.spec.ts:108:9
...
Locator: locator('iframe[name="htmlComp-iframe"]').contentFrame().locator('[data-testid="new-session-click"]')
Expected: visible
Received: hidden
Call log:
  - expect.toBeVisible with timeout 5000ms
  - waiting for locator('iframe[name="htmlComp-iframe"]').contentFrame().locator('[data-testid="new-session-click"]')
    8 × locator resolved to <button data-testid="new-session-click" class="bg-transparent border-0 p-0 m-0 new-session-start-btn">…</button>
      - unexpected value "hidden"

    at chatbotPage.NewsessionChatbotPage (D:\Playwright\Bluedrop_academy\tests\Pages\chatbotPage.ts:120:70)
    at D:\Playwright\Bluedrop_academy\tests\Main.spec.ts:109:27
```
