...
- Name: BlueDrop test cases >> Chatbot Screen >> TC_13: Create new session using "edit" icon button
- Location: D:\Playwright\Bluedrop_academy\tests\Main.spec.ts:103:9
...
Call log:
  - waiting for locator('iframe[name="htmlComp-iframe"]').contentFrame().locator('[data-testid="new-session-click"]')
    - locator resolved to <button data-testid="new-session-click" class="bg-transparent border-0 p-0 m-0 new-session-start-btn">…</button>
  - attempting click action
    ...
    at chatbotPage.NewsesionChatbotPage (D:\Playwright\Bluedrop_academy\tests\Pages\chatbotPage.ts:149:64)
    at D:\Playwright\Bluedrop_academy\tests\Main.spec.ts:104:7
```
...
```ts
   49 |   }
   50 |
   51 |   async SubmitbtnActive() {
   52 |     const frameLocator = this.page.frameLocator(Chatbotlocator.iframeName);
   53 |      this.userMessage = generateRandomQuestion();
   54 |
   55 |     await this.page
   56 |     .frameLocator(Chatbotlocator.iframeName)
   57 |     .getByTestId('seach-msg-input')
   58 |     .fill(this.userMessage);
   59 |     const SubmitBtnEnable = frameLocator.locator(Chatbotlocator.SubmitBtn);
