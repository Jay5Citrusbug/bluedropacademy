...
- Location: D:\Playwright\Bluedrop_academy\tests\Main.spec.ts:90:9

# Error details

```
Error: expect.toBeVisible: Error: strict mode violation: locator('[data-testid="like-thumb"]') resolved to 2 elements:
    1) <span data-testid="like-thumb">…</span> aka getByTestId('like-thumb').first()
    2) <span data-testid="like-thumb">…</span> aka getByTestId('like-thumb').nth(1)
...
Call log:
  ...
  - waiting for locator('iframe[name="htmlComp-iframe"]').contentFrame().locator('[data-testid="like-thumb"]')
    ...
    at D:\Playwright\Bluedrop_academy\tests\Main.spec.ts:91:27
```
...
> 199 |     await expect(frameLocator.locator(Chatbotlocator.LikeBtn)).toBeVisible();
  ...
  223 |     const predefinedBtn = frameLocator.getByRole('button', { name: 'Explain in brief and keep it' });
  224 |
